...
- `output_path` — absolute path where the report will be written. Caller (e.g. `/research` command) resolves and creates the parent directory.

If `output_path` is missing, halt and request it from the caller before starting research.
