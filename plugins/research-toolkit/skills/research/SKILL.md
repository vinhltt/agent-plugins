...
1. **Search Strategy**:
   - **Gemini Toggle**: Read `.agent-plugins.json` for `skills.research.useGemini`. Resolution chain: `<CWD>/.claude/.agent-plugins.json` → `~/.claude/.agent-plugins.json` → default `false`. If `false` or absent, skip Gemini and use WebSearch directly.
   - **Gemini Model**: Read `gemini.model` from the same resolution chain (default: `gemini-3-flash-preview`).
   - If `useGemini` is `true`: first validate Gemini CLI works by running `command -v gemini && echo "ping" | timeout 15 gemini -y -m <gemini.model>`. If validation fails or times out, fall back to WebSearch and warn: "Gemini CLI unavailable, falling back to WebSearch. Set `skills.research.useGemini: false` in `.agent-plugins.json` to suppress this check."
...
**Notes:**
- Research reports are saved to a caller-provided absolute `output_path`. Caller (e.g. `/research` command or `researcher` agent) MUST resolve and pass `output_path` at invocation.
- Default convention used by `/research` command: `./plans/reports/researcher-{YYMMDD-HHMM}-{slug}.md`. Caller is responsible for path resolution and auto-creates the parent directory if missing.
- If `output_path` is missing, ask main agent to supply one before proceeding.
...
- If invocation context includes `output_path` → use it (auto-mkdir parent if missing).
- Else → ask caller to provide `output_path` before continuing.
